...
export class DownloadService {
     ...
     * ```
     */
    async validateAndSelectSize(photoId, imageData, preferredSize) {
        ...
        if (preferredIndex === -1) {
            console.log(chalk.yellow(`   ⚠️  Unknown size '${preferredSize}', defaulting to original`));
            return { selectedSize: "original", selectedWidth: null };
        }
        const validPreferredSize = preferredSize;
        ...
        if (!hasImageDimensions) {
            ...
            console.log(chalk.gray(`   📏 No dimension data available, using ${preferredSize} size`));
            return {
        ...
        if (!requestedWidth || requestedWidth >= originalWidth) {
            if (requestedWidth && requestedWidth > originalWidth) {
                console.log(chalk.yellow(`   📏 Requested ${preferredSize} size (${requestedWidth}px) larger than original (${originalWidth}px), using original`));
                return { selectedSize: "original", selectedWidth: null };
            }
            return {
                selectedSize: validPreferredSize,
        ...
        console.log(chalk.yellow(`   📏 Preferred ${preferredSize} size (${requestedWidth}px) not available for ${photoId} (original: ${originalWidth}px)`));
        ...
        for (let i = preferredIndex + 1; i < sizeOrder.length; i++) {
            ...
            if (!alternativeWidth || alternativeWidth <= originalWidth) {
                console.log(chalk.gray(`   📏 Falling back to ${alternativeSize} size${alternativeWidth ? ` (${alternativeWidth}px)` : ""}`));
                return {
        ...
        console.log(chalk.gray(`   📏 No suitable size found in fallback chain, using original`));
        return { selectedSize: "original", selectedWidth: null };
    }
    /**
    ...
    async extractIxid(photoId) {
        ...
        if (!ixid) {
            try {
                ...
                console.log(chalk.yellow(`   Could not extract ixid from page content: ${errorMessage}`));
            }
        }
        if (!ixid) {
     ...
     * ```
     */
    async downloadImageDirect(photoId, imageData) {
        try {
            ...
            const { selectedSize, selectedWidth } = await this.validateAndSelectSize(photoId, imageData, preferredSize.toLowerCase());
            ...
            if (selectedWidth) {
                ...
                console.log(chalk.gray(`   Using ${selectedSize} size (w=${selectedWidth})`));
            }
            else {
                console.log(chalk.gray(`   Using ${selectedSize} size (no width limit)`));
            }
            console.log(chalk.gray(`   Looking for download button with URL: ${downloadUrl}`));
            const page = await this.browser.getPage();
            ...
            if (!buttonExists) {
                console.log(chalk.yellow(`   Download button not found, trying partial URL match...`));
                ...
                console.log(chalk.gray(`   Found download button with partial URL match`));
     ...
     * ```
     */
    async downloadImage(photoId, imageData, attempt = 1) {
        try {
            const retries = this.config.get("retries");
            if (retries === undefined) {
                throw new Error("Retries not configured");
            }
            console.log(chalk.blue(`📥 [${attempt}/${retries}] Downloading: ${photoId}`));
            ...
            console.log(chalk.gray(`   ✅ Direct download URL worked for ${photoId}`));
            // Save the download using file system service
            const fileInfo = await this.fs.saveDownload(download, photoId);
            console.log(chalk.green(`✅ Downloaded: ${fileInfo.filename} (${(fileInfo.size / 1024 / 1024).toFixed(2)} MB)`));
            this.stats.incrementDownloaded();
            return {
                success: true,
                photoId,
            ...
            console.error(chalk.red(`❌ Error downloading ${photoId}:`, errorMessage));
            // Take debug screenshot if enabled
            await this.takeDebugScreenshot(photoId, attempt);
            const retries = this.config.get("retries");
            if (retries && attempt < retries) {
                console.log(chalk.yellow(`🔄 Retrying ${photoId} (attempt ${attempt + 1}/${retries})`));
     ...
     * ```
     */
    async takeDebugScreenshot(photoId, attempt) {
        ...
        try {
            ...
            console.log(chalk.gray(`   Could not save debug screenshot: ${screenshotError.message}`));
        }
    }
    /**
    ...
    async downloadImages(imageEntries) {
        console.log(chalk.blue(`📦 Processing ${imageEntries.length} images sequentially`));
        const results = [];
     ...
     * ```
     */
    async downloadImagesWithProgress(imageEntries) {
