...
export class OutputFormatter {
    ...
    displayConfiguration(options) {
        ...
        const configItems = [
            ...
            { label: "Preferred size", value: options.preferredSize },
            {
                label: "Image limit",
     ...
     */
    displayEnvironmentCheck(checkResult) {
     ...
     */
    displayEnvironmentSummary(checkResult) {
        const { summary } = checkResult;
        console.log(chalk.blue("\n📊 Summary:"));
        console.log(chalk.gray(`   • Total variables: ${summary.total}`));
        console.log(chalk.gray(`   • Variables set: ${summary.set}/${summary.total}`));
        console.log(chalk.gray(`   • Required set: ${summary.requiredSet}/${summary.required}`));
        console.log(chalk.gray(`   • Optional set: ${summary.optionalSet}/${summary.optional}`));
    }
    /**
     * Display environment check notes
     */
    displayEnvironmentNotes() {
        const notes = [
            "UNSPLASH_EMAIL and UNSPLASH_PASSWORD are optional but enable auto-login",
     ...
     */
    displayEnvironmentStatus(isValid) {
        if (isValid) {
     ...
     */
    displayImageListHeader(imageCount, manifestPath, generatedAt) {
     ...
     */
    displayImageEntry(index, photoId, imageData) {
        ...
        console.log(`     ${chalk.gray("Description:")} ${imageData.description || "No description"}`);
        const sizeInfo = imageData.width && imageData.height
     ...
     */
    displaySectionHeader(title, icon = null) {
