...
export class AuthenticationService {
    ...
    async attemptLogin() {
        ...
        try {
            ...
            console.error(chalk.red("❌ Error during login attempt:"), error.message);
            return false;
        }
    }
    /**
    ...
    async checkIfAlreadyLoggedIn() {
        try {
            ...
            const loginSignupLink = page.locator('a[aria-label="Log in / Sign up"][href="/login"]');
            return !(await loginSignupLink
                .isVisible({ timeout: 3000 })
                .catch(() => false));
        }
        catch (error) {
            console.log(chalk.yellow(`Warning checking login status: ${error.message}`));
            return false;
        }
    }
    /**
    ...
    async handleMissingCredentials() {
        console.log(chalk.yellow("⚠️  No credentials found in environment variables."));
        console.log(chalk.yellow("⚠️  Set UNSPLASH_EMAIL and UNSPLASH_PASSWORD to enable auto-login."));
        if (!this.config.get("headless")) {
            ...
            console.log(chalk.yellow("⏳ Waiting for manual login... (Press Enter when done)"));
            ...
            console.log(chalk.yellow("⚠️  Running in headless mode without credentials."));
    ...
    async performAutomaticLogin(credentials) {
        try {
            ...
            const loginSubmitButton = page
                .locator('button:has-text("Login"), button[type="submit"]:has-text("Login"), input[type="submit"][value="Login"]')
                .first();
            ...
            console.log(chalk.yellow("⚠️  Auto-login failed:", error.message));
