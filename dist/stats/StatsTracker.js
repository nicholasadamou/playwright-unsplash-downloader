...
export class StatsTracker {
    ...
    displayConfiguration(config) {
        ...
        console.log(chalk.gray(`   • Download directory: ${config.get("downloadDir")}`));
        ...
        console.log(chalk.gray(`   • Preferred size: ${config.get("preferredSize")}`));
        console.log(chalk.gray(`   • Image limit: ${config.get("limit") ? config.get("limit") : "No limit"}`));
    }
    /**
    ...
    displayResults(totalStorageBytes = 0) {
        ...
        console.log(chalk.green(`   ✅ Successfully downloaded: ${this.stats.downloaded}`));
        console.log(chalk.gray(`   ⏭️  Skipped (already exist): ${this.stats.skipped}`));
        console.log(chalk.red(`   ❌ Failed to download: ${this.stats.failed}`));
        console.log(chalk.blue(`   ⏱️  Total time: ${this.getElapsedTime().toFixed(2)}s`));
    ...
    displayProgress(currentItem, photoId) {
        ...
        console.log(chalk.blue(`[${processed}/${this.stats.total}] (${progress}%) Processing: ${photoId}`));
    }
    /**
    ...
    displayCompletion() {
        if (this.stats.failed > 0) {
            console.log(chalk.yellow("⚠️  Download process completed with some failures"));
        }
        else {
